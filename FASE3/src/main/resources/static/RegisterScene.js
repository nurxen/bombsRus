class RegisterScene extends Phaser.Scene{
	...
	create() {
		...
		this._createStartButtonOnline(); // Crear botón de inicio local
	...
	// Crear el fondo de la escena
	   ...
	   _createStartButtonOnline() {
	       this.startButtonO = this.add.image(1040, 600, 'StartButton')
	           ...
	           .setOrigin(0.5, 0.5)
	           .setInteractive() // Hacer el botón interactivo
	           .on('pointerdown', () => this._startRegister()); // Llamar a la función para iniciar el juego
	   ...
	   _popUp(){
		...
		document.body.appendChild(formContainer);
		    ...
		    document.getElementById('submitLoginBtn').addEventListener('click', async () => {
		        ...
		        // Aquí puedes hacer una validación contra el archivo .txt o la base de datos simulada
		        try {
		            const response = await fetch('/api/usuario', {
		                method: 'GET', // Asegúrate de tener un endpoint GET en el backend para verificar usuarios
		                headers: { 'Content-Type': 'application/json' },
		            });
		            const users = await response.json();
		            if (users[username] === password) {
		                alert('Inicio de sesión exitoso.');
						document.body.removeChild(formContainer); //cierra pop up
						this.scene.start('GameScene'); //cambia de escena
